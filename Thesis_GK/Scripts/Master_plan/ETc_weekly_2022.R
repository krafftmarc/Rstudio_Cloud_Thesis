...
data <- read_csv("~/Library/Mobile Documents/com~apple~CloudDocs/R/R/Version Control/Heat_Water_Acclimation_Masters_Thesis/Data/Tyree_2022/Tyree LiCor Data 2022/Master_plan/CIMIS_2023_apr_sep.csv")

# Rename the ETo column
data$ETo <- data$Eto
...
# Filter data from April 1st to September 15th, 2023
data <- filter(data, Date >= as.Date("2023-04-01") & Date <= as.Date("2023-09-15"))

# Define Kc values for different periods
data <- data %>%
  mutate(Kc = case_when(
    Date < as.Date("2023-05-01") ~ 0.3,
    Date < as.Date("2023-06-15") ~ 0.54,
    Date < as.Date("2023-08-15") ~ 0.75,
    TRUE ~ 0.65
  ))
...
# Plot the weekly ETc gallons as a bar graph
weekly_plot <- ggplot(data, aes(x = Week, y = Weekly_ETc_gallons)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Weekly ETc Gallons per Acre from April 1st to September 15th, 2023",
       x = "Week",
       y = "Gallons per Acre") +
  theme_minimal() +
...
cumulative_plot <- ggplot(data, aes(x = Week, y = cumulative_ETc_gallons)) +
  ...
  labs(title = "Cumulative ETc Gallons per Acre from April 1st to September 15th, 2023",
       x = "Week",
